...
- `ADMIN_USERNAME` / `ADMIN_PASSWORD` 会在服务首次启动时写入 SQLite，作为 admin 管理员账号。
- 普通用户可在登录页自助注册，注册后立即可登录使用。
- 登录支持邮箱或电话任选其一加密码；admin 可以在管理模块禁用用户或重置普通用户密码。
- 每个账号的任务、图片、缩略图和策划历史按用户隔离。
- 使用统计会记录 AI 代理调用次数、成功/失败、生成图片数和上游返回的 token 用量；普通用户只能看自己，admin 可看全部。
...
页面打开后，可使用 `.env` 中的管理员账号登录，也可以创建普通用户账号。管理员登录后，页面右上角会出现“管理”入口；普通用户登录后会出现“统计”入口。
