...
const SIX_VIEW_REFERENCE_GUARD = [
  ...
  '- If the six-view reference shows a curved smoked transparent lid, rounded lid corners, raised cover, arched cover, hinge curve, or translucent lid lip, keep the exact lid curvature, corner radius, thickness, hinge geometry, opening angle, and smoky transparency. Do not flatten, straighten, square off, or turn the lid into a flat rectangular sheet.',
  '- If the image task asks for a movable or temporary state such as open lid, ice inside, liquid, contents, accessories, hand-free use, or a lifestyle scene, change only that movable/temporary state. Keep the permanent body shape, depth, width, height, side panels, vents, feet, control panel, lid hardware, lid curvature, handles, and seams locked to the six-view reference.',
