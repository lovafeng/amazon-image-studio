...
const STYLE_CANDIDATE_SCHEMA = {
  ...
  properties: {
    ...
    prompt: {
      ...
      description: 'Professional English prompt for a visual style reference board. Never include Chinese characters.',
    },
...
function buildListingPlannerInstructions(baseDraft: AmazonPromptDraft) {
  return [
    ...
    'For each styleCandidates.prompt, describe a visual style reference board with visible typography samples, color palette swatches, lighting/material samples, background/material texture samples, product-finish detail samples, and icon/callout treatment.',
