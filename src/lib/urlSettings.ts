...
function getProfileUrlConnectionKey(profile: Pick<AppSettings['profiles'][number], 'provider' | 'baseUrl' | 'model' | 'apiMode' | 'streamImages' | 'streamPartialImages'>) {
  return JSON.stringify([
    profile.provider,
    profile.baseUrl.trim().replace(/\/+$/, '').toLowerCase(),
    profile.model.trim(),
...
function stripUrlProfileApiKeys(profiles: unknown) {
...
function pickUrlSettingsPayload(value: unknown): unknown | null {
  ...
  return {
    ...
    profiles: stripUrlProfileApiKeys(record.profiles),
  }
}
...
export function buildSettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
  ...
  const hasLegacyOpenAIParams = apiUrlParam !== null || codexCliParam !== null || apiMode !== undefined || modelParam !== null || streamImagesParam !== null || streamPartialImagesParam !== null
  ...
  if (hasLegacyOpenAIParams) {
    ...
    const existingProfile = settings.profiles.find((item) => getProfileDedupKey(item) === getProfileDedupKey(profile))
      ?? settings.profiles.find((item) => getProfileUrlConnectionKey(item) === getProfileUrlConnectionKey(profile))
    if (existingProfile) {
